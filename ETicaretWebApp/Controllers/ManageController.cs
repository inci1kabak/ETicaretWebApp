...
namespace ETicaretWebApp.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            await UserManager.SetTwoFactorEnabledAsync(HttpContext.User.Identity.GetUserId(), true);
            var User = await UserManager.FindByIdAsync(HttpContext.User.Identity.GetUserId());
            if (User != null)
            {
                await SignInManager.SignInAsync(User, isPersistent: false, rememberBrowser: false);
        ...
        {
            await UserManager.SetTwoFactorEnabledAsync(HttpContext.User.Identity.GetUserId(), false);
            var User = await UserManager.FindByIdAsync(HttpContext.User.Identity.GetUserId());
            if (User != null)
            {
                await SignInManager.SignInAsync(User, isPersistent: false, rememberBrowser: false);
        ...
        {
            var result = await UserManager.SetPhoneNumberAsync(HttpContext.User.Identity.GetUserId(), null);
            if (!result.Succeeded)
            {
        ...
        public async Task<ActionResult> ManageLogins(ManageMessageId? message)
        {
            ...
            var userLogins = await UserManager.GetLoginsAsync(HttpContext.User.Identity.GetUserId());
